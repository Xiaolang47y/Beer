...
from core.style_helpers import ensure_arrow_icons
...
class MainWindow(QMainWindow):
    ...
    def apply_theme(self):
        ...
        if theme == 'dark':
            ...
            resolved_theme = 'dark' if self._is_system_dark() else 'light'
        
        arrow_paths = ensure_arrow_icons(resolved_theme)
        if resolved_theme == 'dark':
            self.setStyleSheet(self._get_dark_stylesheet(arrow_paths))
        else:
            self.setStyleSheet(self._get_light_stylesheet(arrow_paths))
    ...
    def _get_dark_stylesheet(self, arrow_paths=None):
        """Get dark theme stylesheet"""
        arrow_paths = arrow_paths or {}
        up_url = arrow_paths.get('up', '')
        down_url = arrow_paths.get('down', '')

        arrow_styles = ""
        if up_url:
            arrow_styles += f"""
            QSpinBox::up-arrow {{ image: url({up_url}); }}
            """
        if down_url:
            arrow_styles += f"""
            QSpinBox::down-arrow {{ image: url({down_url}); }}
            QComboBox::down-arrow {{ image: url({down_url}); }}
            """
    ...
    def _get_light_stylesheet(self, arrow_paths=None):
    ...
    def refresh_ui(self):
        ...
        # Refresh pages (safe: check each one exists before calling)
        for attr in ('main_page', 'saved_page', 'log_panel', 'settings_page'):
